"""
TODO: implement a more scalable and configurable codebase. 
...
def load_data(session: neo4j.Session, path: str, dataset: str = "autism") -> None:
    ...
    parse_df(get_user(session)).write_csv(os.path.join(path, f"{dataset}.user"), separator="\t")
    parse_df(get_item(session)).write_csv(os.path.join(path, f"{dataset}.item"), separator="\t")
    parse_df(get_user_link(session)).write_csv(os.path.join(path, f"{dataset}.user_link"), separator="\t")
    parse_df(get_item_link(session)).write_csv(os.path.join(path, f"{dataset}.item_link"), separator="\t")
